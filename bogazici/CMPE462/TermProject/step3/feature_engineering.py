"""
    In this Script we made feature engineering for different set of features
    To change the feature set, comment out the features that will be removed
    Accuracy, classification report and confusion matrix are reported
    Feature Engineering is done on XGBoost
"""
...
if __name__ == "__main__":
    ...
    print("Train Report:\n",classification_report(y_train,nbsvm.predict(train_bow)))
    print("Validation Report:\n",classification_report(y_val,nbsvm.predict(val_bow)))

    plot_confusion_matrix(confusion_matrix(y_train,nbsvm.predict(train_bow)),confusion_matrix(y_val,nbsvm.predict(val_bow)))
    plt.show()
    ...
    # Apply ANOVA-F Feature Selection to Title Embeddings
    ...
    # Title Embeddings
     = np.array(list(train_df[train_df["class"] == "P"]["title_embeddings"]))
    ...
    ############################################################################
    # Generate Features with SVD
    _,_,P_VT = np.linalg.svd(pos_title_embed)
    _,_,Z_VT = np.linalg.svd(net_title_embed)
    _,_,N_VT = np.linalg.svd(neg_title_embed)

    ############################################################################
    # Comment out the features that must be added from both sides

    X_train_new = np.hstack((
        X_train_new_,
        #cosine_similarity(np.array(list(train_df["title_embeddings"])),P_VT[:5,:]),
        #cosine_similarity(np.array(list(train_df["title_embeddings"])),Z_VT[:5,:]),
        #cosine_similarity(np.array(list(train_df["title_embeddings"])),N_VT[:5,:]),
        #cosine_similarity(np.array(list(train_df["title_embeddings"])),P_VT[:5,:]) - cosine_similarity(np.array(list(train_df["title_embeddings"])),Z_VT[:5,:]),
        #cosine_similarity(np.array(list(train_df["title_embeddings"])),P_VT[:5,:]) - cosine_similarity(np.array(list(train_df["title_embeddings"])),N_VT[:5,:]),
        #cosine_similarity(np.array(list(train_df["title_embeddings"])),Z_VT[:5,:]) - cosine_similarity(np.array(list(train_df["title_embeddings"])),N_VT[:5,:])
        lf_t.transform(np.array(list(train_df["title_embeddings"]))),
        #feature_sel_embed_t.transform(np.array(list(train_df["title_embeddings"]))),
        title_pca.transform(title_scaler.transform(feature_sel_embed_t.transform(np.array(list(train_df["title_embeddings"]))))),
        ################################################################################
        #np.array(list(train_df["sentence_embeddings_mean"])),
        feature_sel_embed_sen_t.transform(np.array(list(train_df["sentence_embeddings_mean"]))),
        #body_pca.transform(body_scaler.transform(feature_sel_embed_sen_t.transform(np.array(list(train_df["sentence_embeddings_mean"]))))),
        lf_te.transform(np.array(list(train_df["sentence_embeddings_mean"]))),
        train_kmeans_title,
        train_kmeans_sent,  
        ##################################################################################
        #np.vstack((np.array(train_df["pos_score"]),np.array(train_df["neg_score"]),np.array(train_df["diff_score"]))),
        #np.vstack((np.array(train_df["body_mean"]),np.array(train_df["body_std"]))),
    ))
        
    X_val_new = np.hstack((
        X_val_new_,
        #cosine_similarity(np.array(list(val_df["title_embeddings"])),P_VT[:5,:]),
        #cosine_similarity(np.array(list(val_df["title_embeddings"])),Z_VT[:5,:]),
        #cosine_similarity(np.array(list(val_df["title_embeddings"])),N_VT[:5,:]),
        #cosine_similarity(np.array(list(val_df["title_embeddings"])),P_VT[:5,:]) - cosine_similarity(np.array(list(val_df["title_embeddings"])),Z_VT[:5,:]),
        #cosine_similarity(np.array(list(val_df["title_embeddings"])),P_VT[:5,:]) - cosine_similarity(np.array(list(val_df["title_embeddings"])),N_VT[:5,:]),
        #cosine_similarity(np.array(list(val_df["title_embeddings"])),Z_VT[:5,:]) - cosine_similarity(np.array(list(val_df["title_embeddings"])),N_VT[:5,:])
        lf_t.transform(np.array(list(val_df["title_embeddings"]))),
        #feature_sel_embed_t.transform(np.array(list(val_df["title_embeddings"]))),
        title_pca.transform(title_scaler.transform(feature_sel_embed_t.transform(np.array(list(val_df["title_embeddings"]))))),
        ##################################################################################
        #np.array(list(val_df["sentence_embeddings_mean"])),
        feature_sel_embed_sen_t.transform(np.array(list(val_df["sentence_embeddings_mean"]))),
        #body_pca.transform(body_scaler.transform(feature_sel_embed_sen_t.transform(np.array(list(val_df["sentence_embeddings_mean"]))))),
        lf_te.transform(np.array(list(val_df["sentence_embeddings_mean"]))),
        val_kmenas_title,
        val_kmenas_sent,
        ##################################################################################
        #np.vstack((np.array(val_df["pos_score"]),np.array(val_df["neg_score"]),np.array(val_df["diff_score"]))),
        #np.vstack((np.array(val_df["body_mean"]),np.array(val_df["body_std"]))),
    ))
    ...
    plot_confusion_matrix(confusion_matrix(y_train,xgbcls.predict(train_bow)),confusion_matrix(y_val,xgbcls.predict(val_bow)))
    plt.show()

    plt.bar(range(len(xgbcls.feature_importances_)), xgbcls.feature_importances_)
