...
if __name__ == '__main__':
    ...
    total_list: List[Tuple[float, float]] = []
    total_avg_list: List[Tuple[float, float]] = []
    alpha_values: np.ndarray = np.arange(0.05, 1.0, 0.05)
    for i in range(len(alpha_values)):
        total = 0
        total_avg = 0
        ALPHA = round(alpha_values[i], 2)
        for query_url in queries:
            ...
            for unique_link in book_dict:
                ...
                tf_dict[unique_link] = (
                calculate_tf_weight(book_dict[unique_link]["tokens"]), book_dict[unique_link]["title"])
            ...
            try:
                ...
                try:
                    precision, avg_precision = calculate_precision(recommended_urls, recommendations)
                    total += precision
                    total_avg += avg_precision
                except ZeroDivisionError as ex:
                    pass
                    #logging.error("No matches founded for the value of alpha {0}".format(ALPHA))
            except Exception as ex:
                logging.error(
                    "Error happened while calculating the simularity results of query url: {0}{1}".format(query_url,
                                                                                                          ex))

        total_list.append((total / len(queries), ALPHA))
        total_avg_list.append((total_avg / len(queries), ALPHA))
        ALPHA += 0.05
    total_list.sort(key=lambda x: x[0], reverse=True)
    total_avg_list.sort(key=lambda x: x[0], reverse=True)
    with open("output/avg_precision_4.txt", "w") as file:
        for pair in total_avg_list:
            file.write(str(pair) + "\n")
    with open("output/precision_4.txt", "w") as file:
